...
if __name__ == "__main__":
    ...
    dataset = core_lzj.UnetDatasetGastric(fs_dir=fs, lipids_dir=lipids, protein_dir=protein, transform=transform)
    print('dataset length is', dataset.__len__())
    # a = dataset[0]
    # im1, im2, im3 = dataset[1]
    # plt.imshow(im1.numpy()[0], cmap='gray')
    # plt.show()
    train_dataset = Subset(dataset, list(range(20, 100)))
    valid_dataset = Subset(dataset, list(range(0, 20)))
    ...
    model_name = 'U-net-two-gastric-batchnorm'
    t1loss, t2loss = [], []
    v1loss, v2loss = [], []
    save_path = './time_' + time_all + '_' + model_name + '_' + opt + '_' + base_name + '/'
    file = save_path + model_name + '_epochs_' + str(epochs) + '.dat'
